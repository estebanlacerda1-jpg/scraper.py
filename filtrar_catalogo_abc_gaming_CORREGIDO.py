...
#   4. Todas las salidas finales quedan en .csv en vez de .xlsx, para poder
#      importarlas directo a Supabase/Lovable.
#
...
for categoria, nombre_archivo in [
    ("MONEDA", "monedas.csv"),
    ("SUSCRIPCION", "suscripciones.csv"),
    ("GIFTCARD", "giftcards.csv"),
]:
    sub = df_clasificado[df_clasificado["Categoria"] == categoria].drop(columns=["Categoria"])
    sub.to_csv(os.path.join(OUTPUT_DIR, nombre_archivo), index=False, encoding="utf-8")
...
# --- guardar todo, en CSV ---
best.to_csv(os.path.join(OUTPUT_DIR, "catalogo_juegos_completo.csv"), index=False, encoding="utf-8")
redes.to_csv(os.path.join(OUTPUT_DIR, "juegos_para_redes.csv"), index=False, encoding="utf-8")
web.to_csv(os.path.join(OUTPUT_DIR, "juegos_para_web.csv"), index=False, encoding="utf-8")
