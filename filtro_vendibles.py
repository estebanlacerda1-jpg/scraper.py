...
Nintendo Switch, y genera tres salidas listas para usar:

    1. vendibles.csv          -> planilla filtrada con el MEJOR precio de cada
                                  juego en tendencia que SÍ está en tu catálogo.
    2. posts_redes.txt        -> textos ya redactados, uno por juego, listos
                                  para copiar/pegar en WhatsApp o Instagram.
    3. demanda_abc_gaming.csv -> demanda (0-100) por franquicia, para usar en
                                  el filtro comercial.
...
CÓMO CORRERLO
--------------
    pip install rapidfuzz requests pandas --break-system-packages
...
solo hay que tocar esa función — el resto del script (matching, csv,
posts) no se toca.
"""
...
def generar_csv(df_resultado, ruta_salida):
...
def main():
    ...
    ap.add_argument("--salida-csv", default="vendibles.csv")
    ...
    generar_csv(df_resultado, args.salida_csv)
