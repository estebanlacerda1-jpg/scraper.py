...
def actualizar_cotizaciones():
    ...
    try:
        ...
        if data.get("result") == "success":
            ...
            if uyu_por_usd:
                ...
                print("   ATENCIÓN: open.er-api.com no trajo UYU, se usan valores de respaldo")
        else:
            print("   ATENCIÓN: respuesta inesperada de open.er-api.com, se usan valores de respaldo")
    except Exception as e:
        print(f"   ATENCIÓN: no se pudo consultar open.er-api.com ({e}), se usan valores de respaldo")
    ...
    try:
        ...
        print(f"   ATENCIÓN: no se pudo obtener el dólar blue ({e}), se usa ARS oficial/respaldo")

    # ARS_TO_UYU se recalcula al final porque depende de uyu_por_usd
    # (de open.er-api.com) Y de ars_por_usd (blue si se consiguió, si no
    # el oficial que ya viene en la misma respuesta de open.er-api.com)
...
def resolver_moneda(precio_txt, nombre_tienda):
    ...
    # Detectar por dominio argentino en el nombre de tienda

    # (las tiendas argentinas tienen su URL en CATALOGOS con .com.ar)
...
def limpiar_numero(valor_txt):
    ...
    Antes esto se decidía únicamente según la moneda (USD/MXN → punto
    decimal fijo, resto → coma decimal fija), lo que rompía con precios
    como "$151,000.00" (ARS con formato US) o "$34.510" (ARS con miles
    en punto), según el caso.
    """
    ...
    if tiene_punto and tiene_coma:
        ...
        # Solo coma: decimal si el último grupo tiene 1-2 dígitos (59,99);
        # si no, es separador de miles (raro, pero por si acaso)
        ...
        # Solo punto: decimal si el último grupo tiene 1-2 dígitos (9.99);
        # si no, es separador de miles (34.510)
        partes = valor_txt.split(".")
...
def extraer_precio_real(card):
    ...
    de tomar el `.price` completo cuando hay oferta.

    WooCommerce renderiza los descuentos así:
        <span class="price"><del>...$1.200...</del><ins>...$900...</ins></span>
    `card.select_one(".price, .amount, bdi")` devuelve el `.price` entero
    (padre), cuyo texto queda "$ 1.200 $ 900" — y luego limpiar_precio()
    toma el PRIMER número, es decir el precio tachado (original), no el
    precio con descuento que el cliente realmente paga.

    Este helper prioriza el precio dentro de <ins> (oferta vigente) y
    sólo cae al `.price` completo si no hay oferta.
    """
...
def detectar_plataforma(nombre, tienda, link="", tags=""):
    """
    Antes solo miraba nombre+tienda. Ahora también revisa:
      - el link del producto (algunas tiendas ponen la plataforma en el
        slug de la URL aunque no esté en el nombre visible, ej.
        ".../products/among-us-ps5")
      - los tags del producto (en Shopify, muchas tiendas etiquetan el
        producto con la plataforma aunque no esté ni en el nombre ni
        en la colección de la que vino)

    Esto es clave para tiendas donde el nombre visible del producto no
    dice la plataforma (ej. "Among Us" a secas) — el dato real puede
    estar en el link o en los tags, no en el nombre.
    """
...
def scrape_shopify(url, nombre_tienda, paginas=MAX_PAGINAS):
    """
    Antes solo tomaba variants[0]["price"] y el título del producto —
    si la tienda usaba la plataforma como VARIANTE (ej. un mismo
    producto "Among Us" con opciones PS4/PS5/Xbox, cada una con su
    propio precio), esa info se perdía por completo: se veía un solo
    precio (el de la primera variante nomás) y ningún rastro de
    plataforma en el nombre.

    Ahora:
      - Lee "tags" y "product_type" del producto (Shopify a veces
        guarda la plataforma ahí, aunque no esté en el título).
      - Si el producto tiene variantes reales (no solo "Default
        Title"), genera UNA FILA POR VARIANTE, con el nombre de la
        variante pegado al nombre y a los tags — así cada plataforma
        sale con su propio precio real, y detectar_plataforma tiene
        de dónde agarrar el dato.
    """
    productos = []
            ...
            for p in items:
                ...
                try:
                    ...
                    if not variantes_reales:
                        # Producto simple, un solo precio (comportamiento de antes)
                        ...
                        # Producto con variantes reales (posible plataforma
                        # como opción) -> una fila por variante
    ...
    for page in range(1, paginas + 1):
        ...
        try:
            ...
            if not cards:
                ...
                # Antes esto se salía en silencio. Ahora deja rastro para
                # poder diagnosticar sin tener que mirar el sitio en vivo:
                # ¿la página cargó bien? ¿nos bloqueó? ¿el tema cambió?
                titulo = soup.title.get_text(strip=True) if soup.title else "(sin titulo)"
                print(
                    f"   [DIAG {nombre_tienda}] status={r.status_code} "
                    f"bytes={len(r.text)} titulo='{titulo[:60]}' "
                    f"-> ningún selector conocido matcheó (posible cambio de tema o bloqueo)"
                )
...
# Sistema custom — cards son LI con clase d-flex.

# Nombre y precio vienen juntos en el texto del LI.

# Necesita Playwright porque carga con JS.
    ...
    async with async_playwright() as pw:
        ...
        for num in range(1, paginas + 1):
            ...
            try:
                ...
                # Cards: LI con clase que incluye "d-flex no-wrap justify-content-start align-items-center"

                cards = soup.select("li.d-flex.no-wrap.justify-content-start.align-items-center")

                if not cards:

                    # Antes esto se salía en silencio. Diagnóstico para
                    # saber si el sitio cambió de tema o algo más bloquea:
                    titulo = soup.title.get_text(strip=True) if soup.title else "(sin titulo)"
                    print(
                        f"   [DIAG {nombre_tienda}] pagina={num} bytes={len(html)} "
                        f"titulo='{titulo[:60]}' -> selector de cards no matcheó nada"
                    )
                ...
                for c in cards:
                    ...
                    try:
                        ...
                        # Nombre: primera línea que no sea precio
...
# SCRAPER JDP4P5 (juegosdigitalesps4ps5.com)

# Es Tiendanube. Card: div.js-item-product

# Nombre: a.item-link (primer texto)

# Precio: span.js-price-display (precio con descuento)
            ...
            try:
                ...
                for c in cards:
                    ...
                    try:
                        ...
                        # Nombre desde el link del producto
                        ...
                        if link_el and not nombre_el:
                            ...
                            # Extraer solo el texto del nombre, sin precio
                        ...
                        # Precio con descuento (el más bajo mostrado)
                        ...
                        # Detecta el separador decimal por posición en vez de
                        # asumir siempre "punto = miles" (rompía con precios
                        # tipo "$9.99" que sí llevan punto decimal real)
...
# SCRAPER ZONADIGITAL (zonadigitalmd.com)

# Es Angular — solo funciona con Playwright.

# Card: div.item-gift__content

# Nombre: primer texto del div

# Precio: span sin clase (primer valor numérico)
    ...
    async with async_playwright() as pw:
        ...
        for num in range(1, paginas + 1):
            ...
            try:
                ...
                # Scroll para cargar el lazy content de Angular
                ...
                for c in cards:
                    ...
                    try:
                        ...
                        # Nombre: primer texto directo del div
                        ...
                        # Precio: primer SPAN sin clase (el número sin centavos separados)

                        precio_el = c.select_one("span:not([class])")

                        if not precio_el:

                            # Fallback: cualquier span con "$"
...
CATALOGOS = {
    ...
    # ---- URUGUAY JUEGOS DIGITALES (WooCommerce + Playwright) ----
    # requests recibe 404 en GitHub Actions pero la página de error contiene
    # tarjetas de productos, lo que genera conteos falsos. Usamos navegador.
        ...
        async with async_playwright() as pw:
            ...
            for num in range(1, paginas + 1):
                ...
                try:
                    ...
                    response = await page.goto(page_url, wait_until="domcontentloaded", timeout=40000)

                    if response and response.status >= 400:

                        print(f"   [DIAG Playwright {nombre_tienda}] status={response.status} url={page_url}")
    ...
    for nombre_tienda, cfg in CATALOGOS.items():
        ...
        try:
            ...
            if tipo == "shopify":
                ...
                # woocommerce: requests primero, playwright como fallback
